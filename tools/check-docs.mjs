...
const files = ['README.md', 'VERIFICATION.md', 'SECURITY.md', ...readdirSync(resolve(root, 'examples/runbooks')).filter(x => x.endsWith('.md')).map(x => `examples/runbooks/${x}`), ...readdirSync(resolve(root, 'docs')).filter(x => x.endsWith('.md')).map(x => `docs/${x}`)];
let diagrams = 0;
